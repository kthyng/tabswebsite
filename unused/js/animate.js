window.onload = function () {
	...
	var latlng = new L.LatLng(29, -88);
	var timelatlng = new L.LatLng(31, -88);
	var delay = 5; // animation delay (larger is slower)
	...
	function animateLines(data) {
		...
		arrowLength = data.arrow.points["0"].point[0].length;
		timeData = data.arrow.time;
		time = timeData.split(",");

		for (var viewEntryKey in data.arrow.points) {
			var arrowsMap = new Map();
			/* var sizeArray = data.arrow.points[viewEntryKey].point[0].size.split(',');
			var angleArray = data.arrow.points[viewEntryKey].point[0].angle.split(','); */

			var sizeArray = data.arrow.points["0"].point[0].size;
			
			var angleArray = data.arrow.points["0"].point[0].angle;
			
			arrowLength = angleArray.length;
		...
		for (var m = 0; m < pointLength; m++) {
			var n = m.toString();
			pt = arrowsDataMap.get(n).get(0).get(1);
